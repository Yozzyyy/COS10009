...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    @cycle = 0 #cycle count for intervals
    @shape_x = 0 # Initialize shape_x to zero
  ...
  def update
    ...
    @shape_x += 10 # Increment shape_x by 10
  ...
  def draw
    ...
    @font.draw_text("Cycle count: #{@cycle}", 10, 10, ZOrder::TOP, 1.0, 1.0, Gosu::Color::BLACK) #adding cycle count into text using increment 

    # Draw a square at (shape_x, 30) with a size of 50x50 pixels
    Gosu.draw_rect(@shape_x, 30, 50, 50, Gosu::Color::BLUE, ZOrder::MIDDLE) #what shape and size 
    puts("2. In draw\n")
  end
end
